...
*
* 如果请求失败，与【catch方法】注册的事件处理程序被调用,
* 当应用发出一个HTTP请求时，实际上是在创建一个【承诺链】。
*
* CSS样式属性名用“-”连接，而【React内联样式】采用驼峰命名法。
* React内联样式有限制，例如，【伪类】不能被直接使用。
* 传统上，人们认为最好的做法是将CSS与内容（HTML）和功能（JavaScript）完全分开。
* 根据这个老的思想流派，目标是将CSS、HTML和JavaScript写进各自的文件。
* 【事实】上，React的哲学是与此截然相反的。由于将CSS、HTML和JavaScript分离到不同的文件中，
* 在大型应用中似乎不能很好地扩展，React将应用的划分建立在其【逻辑功能实体】的基础上。
